...
begin
	...
	const CUM_TIME_COMPONENTS = ["cum_SWIM", "cum_T1", "cum_BIKE", "cum_T2", "cum_RUN"]
end
...
df = @chain begin 	
	...
	transform(TIME_COMPONENTS => ByRow((times...) -> kv_namedtuple(CUM_TIME_COMPONENTS, cumsum(times))) => AsTable)
end
...
md"""
- M PRO $(@bind M_PRO_START Slider(1:70))
- F PRO $(@bind F_PRO_START Slider(1:1e4))
- M PRE $(@bind M_PRE_START Slider(1:1e4))
- F PRE $(@bind F_PRE_START Slider(1:1e4))
- M OPEN $(@bind M_OPEN_START Slider(1:1e4))
- F OPEN $(@bind F_OPEN_START Slider(1:1e4))
- ATH $(@bind ATH_START Slider(1:1e4))
- CLY $(@bind CLY_START Slider(1:1e4))
"""

# ╔═╡ 69781358-815c-4c8a-8bb9-cd7ddac523b6
begin
	# make_distribution(event) = @chain event begin
	# 	@chain begin # make empty array
	# 		df[!, _]
	# 		maximum
	# 		ceil(Int, _)
	# 		zeros(Int, _)
	...
	# end
	# make_distribution("cum_T1")
	# delta_arrs = (;
	# 	[(event |> Symbol, @chain df[!, event] maximum ceil(Int, _) zeros(Int, _))
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "c3da78051c317a52b322155319c0e765ec00c85b"
